...
The Simple Camera로 촬영한 새 사진만 골라 기존 SimpleCamera 업무사진 수신기로 자동 전송하는 iOS 보조 앱입니다.
...
- The Simple Camera가 원본 사진에 기록한 `Software` 메타데이터를 확인합니다.
...
iOS는 일반 앱의 상시 백그라운드 실행을 허용하지 않습니다. 따라서 단축어의 개인용 자동화에서 `The Simple Camera가 닫힐 때` 이 앱의 `새 SimpleCamera 사진 전송` 동작을 즉시 실행하도록 한 번 연결합니다. 사진 검색·선택·반복 단축키는 필요 없습니다.
