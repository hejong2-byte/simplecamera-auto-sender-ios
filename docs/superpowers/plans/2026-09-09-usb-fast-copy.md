...
3. Add explicit verification of the saved manifest, with separate progress/results, no copy or deletion, and path/volume guards. Retain legacy deletion records compatibility. After a successful copy ask whether to delete the original ZIP; only the user's delete confirmation removes it. Temporary extracted working files are cleaned independently.
