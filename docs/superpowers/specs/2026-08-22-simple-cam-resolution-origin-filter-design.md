...
2. 원본 픽셀 크기가 `6048 × 8064` 또는 회전된 `8064 × 6048`이다.
3. 다음 둘 중 하나에 해당한다.
   - 정규화한 TIFF `Software` 값이 `Simple Camera`이거나 `Simple Camera `로 시작한다.
   - `Software` 식별값이 없더라도, TIFF `Make`/`Model`에 기본 iPhone 카메라를 나타내는 `Apple`과 `iPhone` 조합이 없다.

해상도가 일치해도 `Software`가 Simple Camera가 아니고 `Make`/`Model`이 Apple iPhone 기본 카메라를 나타내면 제외한다. 이 규칙은 일반 카메라의 48MP 사진이 같은 해상도인 경우를 막기 위한 것이다.
...
기존 판별기를 해상도와 TIFF 정보를 함께 읽는 `SimpleCameraPhotoMatcher` 역할로 확장한다. ImageIO에서 픽셀 폭·높이, TIFF `Software`, `Make`, `Model`을 읽고 위 판별 규칙을 한 곳에서 적용한다.
...
- ImageIO가 파일이나 픽셀 크기를 읽지 못하면 해당 실행에서는 일치하지 않은 것으로 취급하되, 마지막 재검색 전에는 영구 제외하지 않는다.
...
- Simple Camera `Software`가 없고 기본 iPhone 카메라 표식도 없으면 목표 해상도 사진을 허용한다.
- 목표 해상도라도 Apple/iPhone 기본 카메라 표식이 있고 Simple Camera `Software`가 없으면 거부한다.
- 다른 해상도, 손상된 파일, 픽셀 정보를 읽을 수 없는 파일은 거부한다.
